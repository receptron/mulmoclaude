...
To share one workspace between two servers anyway, `MULMOCLAUDE_ALLOW_MULTIPLE_INSTANCES=1` restores the old walk-forward behaviour, with the token-stomping it implies. Use the env var here, not the flag: `yarn dev --allow-multiple-instances` does **not** work, because yarn hands trailing args to the last command of a compound script and the guard that stops the launch runs in the first.
