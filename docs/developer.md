...
**Both env vars, not just `PORT`.** A launch that would share this workspace with a server already running against it now stops (#3079):

```
MulmoClaude is already running against this workspace at http://localhost:3001
```

That covers `yarn dev` twice AND `PORT=3100 yarn dev`, which used to be the recommended escape hatch here. It was never a good one: the two instances overwrite each other's `.session-token`, after which a stateless plugin dispatch authenticates against the wrong server while the session-scoped `/api/internal/tool-result` push is dropped — so plugin views silently stop rendering on one of them. Give the second instance its own `MULMOCLAUDE_WORKSPACE_PATH`, as above, and no flag is needed.
