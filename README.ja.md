...
MulmoClaude サーバは起動するたびに新しい bearer トークンを生成して `<workspace>/.session-token`（`$MULMOCLAUDE_WORKSPACE_PATH`、未設定なら `~/mulmoclaude`）に書き出します。bind したポートも同じ場所の `.server-port` に書きます。

**ブリッジは再起動に自力で追従します。** 接続に失敗すると両方のファイルを読み直し、サーバが別世代（新しいトークン、新しいポート、またはその両方）で戻ってきていれば socket を作り直します (#3078)。ブリッジを立て直す必要はありません。

トークンの固定が今も有用なのは、ブリッジが**サーバと別のマシン**で動く場合です（workspace をそもそも読めないため）。サーバとブリッジの両方に同じ長いランダム文字列を `MULMOCLAUDE_AUTH_TOKEN` で渡すと、サーバは自動生成ではなくこの値をそのまま使います。
