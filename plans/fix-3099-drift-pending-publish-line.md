...
- `pending-publish` と `drifted` は **行そのものを一致で断言**する。期待文字列は fixture の
  数から組み立てるので、ハードコードした実装も落ちる。3組（17/4, 9/8, 231/5）で回す
- これは**文言を変えると赤になる**。意図的にそうしている: この行は operator が
  「publish して安全か」を読む行で、**言い換えは判断であって副作用であってはならない**
- 「ok と見分けられる」「drifted と見分けられる」は**一致断言に吸収された**ので、弱い形の
  重複テストとしては残さない —— 期待文字列そのものに `⧗` と
  `bumped but NOT published yet` が入っており、意図はそこで読める
- `ok` と `skipped` は現状の文言を維持（リグレッション防止）
- registry fallback の注記は、それを持つどの verdict でも落ちない
