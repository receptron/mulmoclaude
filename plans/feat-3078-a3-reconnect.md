...
- break-verify: 追従ロジックを無効化すると **8 件中 5 件が赤**（うち 3 件は待ち時間切れという形で。
  作り直しが来ないので assertion まで到達しない）になり、negative control 3 件
  （「作り直さないこと」「既定値に繋がないこと」を主張するケース — 何も作り直さなければ通る）だけ緑。
- **結合実行で回すこと。** `test/bridges/test_*.ts` をまとめて走らせたときだけ出る不具合がある
  （Codex がそれで見つけた）: 単体ファイルで走らせるとプロセスが先に終わるので、後から reject する
  タイマが露見しない。
